...
export const refreshTokenMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (!userId) { // Si le Refresh Token n'est pas valide
            // Supprimer les cookies existants
            res.clearCookie('accessToken');
            res.clearCookie('refreshToken');
        ...
        if (!user || user.user.refreshToken !== refreshToken) { // Si le Refresh Token ne correspond pas à celui stocké en base
            // Supprimer les cookies existants
            res.clearCookie('accessToken');
            res.clearCookie('refreshToken');
            return APIResponse(res, null, 'Invalid Refresh Token.', 403); // Répondre avec une erreur
        ...
        await updateUser(new ObjectId(userId), { refreshToken: newRefreshToken });
